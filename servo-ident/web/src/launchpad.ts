...
const LAUNCHPAD_GROUPS: LpGroup[] = [
  ...
  },
  {
    ...
    macros: [
      ...
      {
        name: "SERVO_TUNE_DYNAMICS",
        blurb: "closed-loop coordinate descent on the dynamics model, scored by transient-window ferr rms",
        params: [
          { name: "TERMS", type: "list", dflt: "MASS,VISCOUS,COULOMB,LEAD", hint: "comma list; DIRECTION_SPLIT also allowed" },
          { name: "MAX_ACCEL", type: "float", dflt: "config", unit: "mm/s²" },
          { name: "MAX_SPEED", type: "float", dflt: "config", unit: "mm/s" },
          { name: "STEP", type: "float", dflt: "0.15", hint: "first probe fraction, 0.02–0.5" },
          { name: "PROFILE", type: "string", hint: "baseline TOML; defaults to the live model, else the node's profile" },
          SERVOS,
          { name: "BOUND", type: "float", dflt: "20", unit: "mm", hint: "XY pattern inset from the configured bounds" },
          { name: "SMALL_SIZE", type: "float", dflt: "20", unit: "mm", hint: "center small-stroke box size" },
          { name: "NAME", type: "string", dflt: "tune" },
        ],
      },
      {
        ...
        params: [
          ...
          { name: "X_PEAK", type: "float", unit: "Hz", hint: "FRF peak from SERVO_MEASURE_COMPLIANCE - with the notch sets pin load fraction 1-(f_b/f_peak)^2" },
          { name: "Y_PEAK", type: "float", unit: "Hz", hint: "FRF peak from SERVO_MEASURE_COMPLIANCE - with the notch sets pin load fraction 1-(f_b/f_peak)^2" },
          { name: "X_ZETA", type: "float", hint: "per-mode predictor belt damping ratio for X; overrides ZETA (finite, > 0, no upper cap)" },
          { name: "Y_ZETA", type: "float", hint: "per-mode predictor belt damping ratio for Y; overrides ZETA (finite, > 0, no upper cap)" },
          { name: "ZETA", type: "float", dflt: "0.02", hint: "shared predictor belt damping ratio fallback for modes without X_ZETA/Y_ZETA" },
      ...
      {
        name: "SERVO_TUNE_PIN",
        blurb: "full measured pin-rotor tuning campaign: per mode measure f_b/f_peak (unless overridden), pin, run a coarse then fine ZETA staircase, then one shared LEAD staircase on the lowest-frequency mode; writes the tuned profile live and prints the ready-to-run SERVO_SET_COMPLIANCE line",
        params: [
          { name: "MODES", type: "string", dflt: "XY", hint: "XY, X or Y - modes to tune" },
          { name: "DWELL", type: "float", dflt: "3", unit: "s", hint: "per staircase step, min 1" },
          { name: "AMPLITUDE", type: "float", dflt: "0.01", unit: "mm" },
          { name: "LEAD_VALUES", type: "list", dflt: "0,150,300,450,600", hint: "PIN_LEAD_US staircase values (us)" },
          { name: "ZETA_COARSE", type: "list", dflt: "0.02,0.035,0.05,0.08,0.12,0.2,0.3", hint: "coarse zeta staircase; the fine stage spans winner/1.6..winner*1.6" },
          { name: "X_FREQ", type: "float", unit: "Hz", hint: "notch f_b for X; give with X_PEAK to skip X's measurement" },
          { name: "Y_FREQ", type: "float", unit: "Hz", hint: "notch f_b for Y; give with Y_PEAK to skip Y's measurement" },
          { name: "X_PEAK", type: "float", unit: "Hz", hint: "FRF peak for X; give with X_FREQ to skip X's measurement" },
          { name: "Y_PEAK", type: "float", unit: "Hz", hint: "FRF peak for Y; give with Y_FREQ to skip Y's measurement" },
          { name: "PROFILE", type: "string", hint: "baseline TOML; defaults to the live model, else the node's profile" },
          { name: "NAME", type: "string", dflt: "pin_tune" },
        ],
      },
      {
